...
const traderClient = new services.TradeClient('localhost:9945', grpc.credentials.createInsecure());
...
export function markets(): Promise<Array<any>> {
  return new Promise((resolve, reject) => {
    traderClient.markets(
      new messages.MarketsRequest(),
      (err, response) => {
        if (err)
          return reject(err);
        const list = response!.getMarketsList()
...
export function balances({ baseAsset, quoteAsset }: { baseAsset: string, quoteAsset: string }): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
    traderClient.balances(
      request,
      (err, response) => {
        if (err)
          return reject(err);

        const baseAmount: number = (response.getBalancesList().find(b => b.getAsset() === baseAsset))!.getAmount();
        const quoteAmount: number = (response.getBalancesList().find(b => b.getAsset() === quoteAsset))!.getAmount();
        const reply = {
          fee: response!.getFee(),
          balances: {
